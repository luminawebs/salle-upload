...
def add_question(q_num, stem, options, is_tf, is_true, feedback=None):
    ...
    print(f"Added Q{q_num}: {BeautifulSoup(stem, 'html.parser').get_text(strip=True)[:60]}... | Options: {len(options)}")
...
for lst in lists:
    ...
    if has_respuesta and is_nested:
        for li in lis:
            ...
            stem_html = process_image_src(li_clone.decode_contents(), None).strip()
            
            # --- NEW ROBUST LOGIC ---
            if not stem_html:
                collected_html = []
                node = lst.previous_sibling
                while node:
                    if node.name in ['ul', 'ol', 'h1', 'h2', 'h3', 'hr']:
                        break
                    if node.name is not None:
                        collected_html.insert(0, str(node))
                    elif str(node).strip():
                        collected_html.insert(0, str(node))
                    node = node.previous_sibling
                raw_stem = "".join(collected_html).strip()
                if raw_stem:
                    stem_html = process_image_src(raw_stem, None)
            # ------------------------
...
print(f"FORMAT B extracted: {len(xml_questions)}")
